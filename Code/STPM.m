mu = 0;
    ...
    %    for j = 1:2;
            ...
            for s = 3.35;%Std
                Zi = -1.5*s:1e-1:1.5*s;%Tuning s or 0.5 changes initial S abundance  
                Zri = normpdf(Zi, mu, s);%Frequency each phenotype
                Zi = Zi + abs(min(Zi));%Move everything to the right.
                SRA = repmat(Zi, [10,1,10]);
...
munew = mean(SRA(1,:,1));
for pA = 1:length(Zi);
N1(pA,2) = exp(-gamma*(Zi(1,pA) - munew)^2);
N1(pA,1) = Zi(1,pA) - munew;
end
subplot(3,2,2)
...
%for s = 100;%Std
%       Zi = -1.5*s:1e-1:1.5*s;%Tuning s or 0.5 changes initial S abundance  
%       Zri = normpdf(Zi, 0, s);%Frequency each phenotype
%       Zi = D + abs(min(Zi));%Move everything to the right.
