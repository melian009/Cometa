...
r=dlmread('Bcor.txt'); %read the empirical network. Format: biadjacency matrix representing a bipartite graph. I assume the matrix has no zeroed row or column. This matrix parameterizes the forbidden links of the network (zeroed elements of the matrix)
mmedio=0.7; %mean level of mutuslism selection
...
theta= thmedio.*rand(dim,1);%sampling the theta-values for each species in the network.
pmedio=1-mmedio;
...
zn=zi; %create an auxiliary vector
...
for timestepsi=1:timesteps %run the simulations
    if bandeira ==0 %is the simulation running?
        %compute the contribution of each interaction to the selection
        %gradient
        for i=1:dim
            for j=1:dim
